...
simFla.ux.plugins.getYPos = function (oElement) {
    ...
    return iReturnValue - Ext.getBody().getScroll().top;
}
...
simFla.ux.plugins.signaturePad = Ext.extend(Ext.util.Observable, {
    ...
    initControl:function () {
        var width = 400, height = 100, style = 'border:1px solid black;';
        ...
        if (this.appDisablesButtons) {
            this.appDisablesButtons = this.disabledButtons = true;
        }
        if (this.browserHasCanvas()) {
            this.cmp.html = '<canvas width="' + width + '" height="' + height + '" style="' + style + '" id="thecanvas"></canvas>';
        } else if (this.appDisablesButtons) {
            this.app.enableButtons();
        }
    },
    browserHasCanvas:function () {
    ...
    initCanvas:function () {
        ...
        if(!this.browserHasCanvas()) return;

        oCanvas = document.getElementById("thecanvas");

        this.enableTouch(oCanvas);
        ...
        if (oCanvas) {
            ...
            oCtx.beginPath();
            //users can set their own line color
            oCtx.strokeStyle = this.color || "rgba(0,0,0, 0.5)";
            oCtx.strokeWidth = "4px";

            this.enableMouseEvents(oCanvas, oCtx);
        }
    },
    ...
    reset:function () {
        ...
        c.beginPath();
        if (this.appDisablesButtons) {
            this.app.disableButtons();
            this.disabledButtons = true;
        }
    },
    enableTouch: function(oCanvas) {
    ...
    enableMouseEvents: function(oCanvas, oCtx) {
        ...
        oCanvas.onmousemove = function (e) {
            if (bMouseIsDown) {
                //allow users to set their own cursor, since default is 'select'
                if (this.activeCursor) e.target.style.cursor = this.activeCursor;

                if (this.parent.appDisablesButtons && this.parent.disabledButtons) {
                    this.parent.app.enableButtons();
                    this.parent.disabledButtons = false;
                }
        ...
        oCanvas.onselectstart = function () {
            return false;
        } // ie kill selection cursor

    },
    enable: function() {
        this.enableTouch(this.oCanvas);
        this.enableMouseEvents(this.oCanvas, this.oCanvas.getContext("2d"));
    },
    disable:function () {
        this.oCanvas.removeEventListener("touchstart", simFla.ux.plugins.touchHandler, true);
        this.oCanvas.removeEventListener("touchmove", simFla.ux.plugins.touchHandler, true);
        this.oCanvas.removeEventListener("touchend", simFla.ux.plugins.touchHandler, true);
        this.oCanvas.removeEventListener("touchcancel", simFla.ux.plugins.touchHandler, true);
        this.oCanvas.onmousedown = null;
        this.oCanvas.onmouseup = null;
        this.oCanvas.onmousemove = null;
    }
});